import React, { useState, useEffect } from 'react'
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';
import { fetchArticles, auth} from '../../../../reducer/actions';
import SignIn from './SignIn'
import SignUp from './SignUp'
import Forgot from './Forgot'

// import { injectGlobal } from 'styled-components'


// injectGlobal`
//     *, ::after, ::before {
//         -webkit-box-sizing: border-box;
//         box-sizing: border-box;
//     }

//     body {
//         font-family: Poppins,Helvetica,sans-serif;
//         font-size: 1rem;
//         font-weight: 400;
//         line-height: 1.5;
//         color: #212529;
//         text-align: left;
//     }

//     body, html {
//         font-size: 13px;
//         font-weight: 300;
//         font-family: Poppins,Helvetica,sans-serif;
//     }

//     body {
//         color: 
//         #646c9a;
//     }
//     button,input{
//         overflow: visible;
//     }

//     button, input, optgroup, select, textarea{
//         margin: 0;
//         font-family: inherit;
//         font-size: inherit;
//         line-height: inherit;
//     }

//     .kt-login.kt-login--v6 .kt-login__aside .kt-login__wrapper .kt-login__container .kt-login__body {
//         width: 100%;
//         -webkit-box-flex: 1;
//         -ms-flex: 1;
//         flex: 1;
//     }

// `
interface AllMode {
    [key: string]: JSX.Element
}

const Auth: React.FC<any> = (props)=>{

const {
    fetchArticles,
    auth,
    } = props;

const [mode, setMode]= useState<string>('SignIn');
const allMode: AllMode = {
    SignIn :    <SignIn setMode={setMode} auth={auth}/>,
    SignUp :    <SignUp setMode={setMode} auth={auth}/>,
    Forgot :    <Forgot setMode={setMode} auth={auth}/>
}

const setModeToDom = (modeWant: string)=>{
    return allMode[modeWant]
}

useEffect(()=>{
    setModeToDom(mode)    
}, [mode])

    return(
        <div className="kt-grid__item  kt-grid__item--order-tablet-and-mobile-2  kt-grid kt-grid--hor kt-login__aside" style={{
            flex: '0 0 auto',
            display: 'flex',
            flexDirection: 'column',
            padding: '2rem',
            background:'#fff',
            width: '600px',
        }}>
    <style dangerouslySetInnerHTML={{__html: `
        .kt-login__head{
            margin: 3rem 0;
            text-align: center
        }
    
        .kt-login__actions {
            margin: 3rem 0;
            text-align: center
        }
    `}} />

            <div className="kt-login__wrapper" style={{
                flex: 1,
                display: 'flex',
                flexDirection: 'column',
                padding: '2rem',
            }}>
                <div className="kt-login__container" style={{
                    flex: 1,
                    width: '400px',
                    margin: '0 auto',
                    display: 'flex',
                    WebkitBoxAlign: 'center',
                    alignItems: 'center',
                    paddingBottom: '5rem'
                }}>
                    <div className="kt-login__body" style={{
                        width: '100%',
                        flex: 1
                    }}>
                        <div className="kt-login__logo" style={{
                            margin: '3rem 0',
                            textAlign: 'center'
                        }}>
                            <a href="#">
                                logo
                            </a>
                        </div>
                        {
                            allMode[mode]
                        }
                    </div>
                </div>
                <div className="kt-login__account" style={{
                    textAlign: 'center',
                }}>
                    <span className="kt-login__account-msg">
                        Don't have an account yet ?
                    </span>&nbsp;&nbsp;
                    <a onClick={()=>{mode === "SignIn"?setMode('SignUp'):setMode('SignIn')}} id="kt_login_signup" className="kt-login__account-link">{mode === "SignUp"?"Sign In!":"Sign Up!"}</a>
                </div>
            </div>
        </div>
    )
}
const mapStateToProps = (state: any) => (state);

const mapDispatchToProps = (dispatch: any) =>
    bindActionCreators({
      fetchArticles,
      auth
    }, dispatch);

export default connect(mapStateToProps, mapDispatchToProps)(Auth)